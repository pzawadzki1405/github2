			...
			try {
				...
				if (context.request.method === 'GET') {
					...
					try{
						...
						var mySearch = search.load({ id: 'customsearch_locked_orders'});
						var results_search = mySearch.run();
						var results_search_array = results_search.getRange({
    					start: 0,
    					end: 100
						});
						//column [0] is document number, column [1] is user name
						var fulfillment_number = 'FUL91634';
						for (var i = 0; i < results_search_array.length; i++) {
								if ((results_search_array[i].getValue(results_search.columns[0])) == fulfillment_number)
  							strOutput = results_search_array[i].getText(results_search.columns[1]) + ' fulfillment id: ' +
								results_search_array[i].getValue(results_search.columns[0]);
						...
						//results_search.each(addResults);
						//strOutput = 'ok';
						//var value_search = '';
						//var resultsSet = mySearch.run();
						//var firstResult = resultsSet.getRange({
				    //  start: 0,
				    //  end: 1
				    //})[0];
						//for (var i = 0; i < results_search.length; i++) {
						//		addResults(results_search[i]);
								//strOutput = results_search[i].getText(results_search.columns[1]);
						//}
						//function addResults(result) {
						//	var value_search ='ok';
        			//var value_search = result.getText({name: 'Set by'});
						//	strOutput = value_search;
						//	return true;
    				//}
					 }
					 catch (err){
						 log.error("Error in run search", err);
					 }
					...
					var form = serverWidget.createForm({
						title: 'Print Integrated Shipping Labels v2'
					});
					form.addField({
					...
					form.addField({
						id: 'custpage_locked_orders',
						type: serverWidget.FieldType.LABEL,
						label: 'Locked orders: ' + strOutput
					});
					form.addField({
							...
							if (ifSearchObj.length > 0) {
								...
								if (validateValue(ifInternalIdValue)) {
									for (var r = 0; r < ifSearchObj.length; r++) {
										...
										if (shipMethodVal == "GLS Express"){
											shipMethodVal = "FedEx Ground® USA"
										}
										...
										if (allowRepeating == 'F') {
											if (itemName == itemNameVal || itemName == itemUpcCode) {
												...
												if (validateValue(itemNameVal) && validateValue(itemName)){
													sublist.setSublistValue({
														id: 'custpage_scan_item_barcode',
														line: r,
														value: itemNameVal
		...
		function getItemId(itemName) {
			try {
				...
				if (searchResults.length > 0) {
					...
					var AliasSearchObj = search.create({
						type: "customrecord_wmsse_sku_alias",
						filters: [
							["name", "is", itemName],
							"OR",
							["custrecord_wmsse_alias_desc", "is", itemName]
						],
						columns: [
							search.createColumn({
								name: "name",
							...
							search.createColumn({
								name: "custrecord_wmsse_alias_item.internalid",
								label: "Item Internal ID"
							}),
						]
					});
					var AliassearchResults = AliasSearchObj.run().getRange({ start: 0, end: 50});
					log.debug("AliassearchResults -->getItemId", AliassearchResults);
				  log.debug("AliassearchResults -->getItemId", AliassearchResults.length);
					if (AliassearchResults.length > 0) {
						var itemId = AliassearchResults[0].getValue({
							name: "custrecord_wmsse_alias_item.internalid",
							label: "Internal ID"
						});
						return itemId;
						...
						log.debug("RETURN NOTHING -->getItemId", AliassearchResults);
						return "";
					}

					var AliasSearch = search.load({ id: 'customsearch4909'});
			...
			var mySearch = search.load({ id: 'customsearch4909'});
			...
			log.debug("results_search_array -->getItemId", results_search_array);
			log.debug("results_search_array -->getItemId", results_search_array.length);
			if (results_search_array.length > 0) {
							var itemId = results_search_array[0].getValue(results_search.columns[1]);
							return itemId;
			}
			else{
				return "";
